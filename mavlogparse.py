...
"""
# imported from https://gist.github.com/ES-Alexander/1498c87ad6c07623f6776c552869c805
...
class Telemetry:
    DEFAULT_FIELDS = {
        ...
        "ATTITUDE": [
            s for rot in ("roll", "pitch", "yaw") for s in (rot, rot + "speed")
        ],
    ...
    def _init_fields(self, fields):
        ...
        for type_, field in fields.items():
            if field is None:
                ...
                fields[type_] = getfullargspec(
                    getattr(mavutil.mavlink, type_class).__init__
                ).args[1:]
    ...
    @staticmethod
    def csv_to_df(
        filename, timestamp="timestamp", timezone="Australia/Melbourne", **kwargs
    ):
        ...
        def parser(utc_epoch_seconds):
            return (
                pd.to_datetime(utc_epoch_seconds, unit="s")
                .tz_localize("utc")
                .tz_convert(timezone)
            )
    ...
    def __create_field_tracker(fields):
        ...
        def get_fields(type_):
            ...
            return {
                field: None
                for field in getfullargspec(
                    getattr(mavutil.mavlink, type_).__init__
                ).args[1:]
            }

        if fields is None:
            fields = {
                t[8:-8].upper(): get_fields(t)
            ...
            with open(fields) as in_file:
                fields = {
                    type_: (
                        {field: None for field in fields_}
                        if fields_
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", help="list useful (non-constant) fields"
    )
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="turn off printed output"
    )
    ...
    if args.list:
        fields = Telemetry.get_useful_fields(
            args.tlogs, args.output, args.fields, args.dialect, verbose
        )
        if verbose:
